...
void main()
{
    ...
    if(displacementMapping > 0.0){
        ...
        float scale=0.2;
        float center=texture2D(displacementTex, texPos).x*0.2;
        ...
        normal=normal+cross(normal,T)*(hu_plus-hu_minus)/(2.0*delta_uv)+cross(S,normal)*(hv_plus-hv_minus)/(2.0*delta_uv);
    }//
